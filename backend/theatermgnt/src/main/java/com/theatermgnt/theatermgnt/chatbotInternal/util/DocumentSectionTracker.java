...
public class DocumentSectionTracker {
    ...
    private static final Pattern CHAPTER_PATTERN =
            Pattern.compile("^\\s*(CHƯƠNG\\s+[IVXLCDM]+)[:\\s]+(.{3,150})$", Pattern.MULTILINE | Pattern.UNICODE_CASE);

    private static final Pattern SUBSECTION_PATTERN = Pattern.compile(
            "^\\s*(\\d+\\.\\d+)\\.?\\s+([A-Za-zÀ-ỹĐđ][^\\n\\r]{2,150})$", Pattern.MULTILINE | Pattern.UNICODE_CASE);
    ...
    public SectionInfo getSectionFromText(String chunkText) {
        ...
        for (SectionInfo section : sections) {
            ...
            String sectionPattern =
                    escapedNumber + "[:\\s\\.]+.*?" + escapedTitle.substring(0, Math.min(20, escapedTitle.length()));
            ...
            try {
                ...
                if (chunkText.contains(section.getSectionNumber())
                        && chunkText.contains(section.getSectionTitle()
                                .substring(
                                        0,
                                        Math.min(15, section.getSectionTitle().length())))) {
                    return section;
                }
            }
        }
