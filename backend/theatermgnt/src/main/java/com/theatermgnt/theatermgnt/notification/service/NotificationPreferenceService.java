...
public class NotificationPreferenceService {
    ...
    public NotificationPreferenceResponse createPreference(NotificationPreferenceRequest request) {
        log.info("Creating preference for recipient: {}, channel: {}", 
                request.getRecipientId(), request.getChannelId());
        
        NotificationChannel channel = channelRepository.findById(request.getChannelId())
                .orElseThrow(() -> new AppException(ErrorCode.CHANNEL_NOT_FOUND));
        
        // Check if preference already exists
        preferenceRepository.findByRecipientIdAndChannelAndCategory(
    ...
    public NotificationPreferenceResponse updatePreference(UpdatePreferenceRequest request) {
        log.info("Updating preference for recipient: {}, channel: {}, category: {}", 
                request.getRecipientId(), request.getChannelName(), request.getCategory());
        
        NotificationChannel channel = channelRepository.findByName(request.getChannelName())
                .orElseThrow(() -> new AppException(ErrorCode.CHANNEL_NOT_FOUND));
        
        NotificationPreference preference = preferenceRepository
                .findByRecipientIdAndChannelAndCategory(
                        request.getRecipientId(),
                        channel,
                        request.getCategory()
                )
    ...
    public boolean isChannelEnabledForUser(String recipientId, String channelName, NotificationCategory category) {
        log.debug("Checking if channel {} is enabled for recipient: {}, category: {}", 
                channelName, recipientId, category);
    ...
    public void createDefaultPreferences(String recipientId, RecipientType recipientType) {
        ...
        List<NotificationPreference> defaultPreferences = createDefaultPreferences(
                recipientId, recipientType, activeChannels);
        
        preferenceRepository.saveAll(defaultPreferences);
        
        log.info("Created {} default preferences for recipient: {}", 
                defaultPreferences.size(), recipientId);
    }
