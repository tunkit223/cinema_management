...
public class NotificationDispatcher {
    ...
    @Async
    @Transactional
    public void dispatch(Notification notification, List<String> channels, Map<String, Object> templateVariables) {
        log.info("Dispatching notification {} to channels: {}", notification.getId(), channels);
        ...
        for (String channelName : channels) {
            try {
                ...
                boolean isEnabled = preferenceService.isChannelEnabledForUser(
                ...
                if (!isEnabled) {
                    log.info("Channel {} is disabled for recipient: {}", 
                            channelName, notification.getRecipientId());
                    saveLog(notification, channelName, "SKIPPED", 
                            Map.of("reason", "Channel disabled by user preference"), null);
                    continue;
                }
                ...
                if (provider == null) {
                    ...
                    saveLog(notification, channelName, "FAILED", 
                            Map.of("reason", "Provider not found"), null);
                    continue;
                }
                ...
                if (!provider.isAvailable()) {
                    ...
                    saveLog(notification, channelName, "FAILED", 
                            Map.of("reason", "Provider not available"), null);
                    continue;
                }
                
                // 3. Build send request
                NotificationSendRequest sendRequest = buildSendRequest(
                        notification, 
                        channelName, 
                        templateVariables
                );
                ...
                // 5. Save log
                saveLog(notification, channelName, result.getStatus(), 
                        result.getProviderResponse(), result.getSentAt());
                ...
                if (result.isSuccess()) {
                    ...
                    log.error("Failed to send notification via {}: {}", 
                            channelName, result.getErrorMessage());
                }
        ...
        log.info("Notification {} dispatch completed. Status: {}", 
    ...
    private NotificationSendRequest buildSendRequest(
        ...
        Account recipient = accountRepository.findById(notification.getRecipientId())
                .orElse(null);
