...
public interface InvoiceRepository extends JpaRepository<Invoice, String> {
    ...
    @Query("SELECT i FROM Invoice i WHERE (LOWER(i.id) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(i.bookingId) LIKE LOWER(CONCAT('%', :search, '%'))) ORDER BY i.createdAt DESC")
    Page<Invoice> searchInvoices(@Param("search") String search, Pageable pageable);

    @Query("SELECT i FROM Invoice i WHERE (LOWER(i.id) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(i.bookingId) LIKE LOWER(CONCAT('%', :search, '%'))) AND i.status = :status ORDER BY i.createdAt DESC")
    Page<Invoice> searchInvoicesByStatus(@Param("search") String search, @Param("status") InvoiceStatus status, Pageable pageable);
    ...
    @Query(value = "SELECT i.* FROM invoices i \n" +
            ...
            countQuery = "SELECT COUNT(*) FROM invoices i \n" +
                    ...
                    "WHERE r.cinema_id = :cinemaId AND i.status = :status",
            nativeQuery = true)
    Page<Invoice> findByCinemaAndStatus(@Param("cinemaId") String cinemaId, @Param("status") String status, Pageable pageable);
    ...
    @Query(value = "SELECT i.* FROM invoices i \n" +
            ...
            countQuery = "SELECT COUNT(*) FROM invoices i \n" +
                    ...
                    "WHERE r.cinema_id = :cinemaId AND i.created_at BETWEEN :startDate AND :endDate",
            nativeQuery = true)
    Page<Invoice> findByCinemaAndDateRange(@Param("cinemaId") String cinemaId, @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate, Pageable pageable);
    ...
    @Query(value = "SELECT i.* FROM invoices i \n" +
            ...
            countQuery = "SELECT COUNT(*) FROM invoices i \n" +
                    ...
                    "WHERE r.cinema_id = :cinemaId AND (LOWER(i.id) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(i.booking_id) LIKE LOWER(CONCAT('%', :search, '%')))",
            nativeQuery = true)
    Page<Invoice> searchInvoicesWithinCinema(@Param("search") String search, @Param("cinemaId") String cinemaId, Pageable pageable);
    ...
    @Query(value = "SELECT i.* FROM invoices i \n" +
            ...
            countQuery = "SELECT COUNT(*) FROM invoices i \n" +
                    ...
                    "WHERE r.cinema_id = :cinemaId AND i.status = :status AND (LOWER(i.id) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(i.booking_id) LIKE LOWER(CONCAT('%', :search, '%')))",
            nativeQuery = true)
    Page<Invoice> searchInvoicesByStatusWithinCinema(@Param("search") String search, @Param("status") String status, @Param("cinemaId") String cinemaId, Pageable pageable);

    // Cinema-scoped statistics methods
    @Query(value = "SELECT COUNT(*) FROM invoices i \n" +
            "JOIN bookings b ON b.id::TEXT = i.booking_id \n" +
            "JOIN screenings s ON s.id = b.screening_id \n" +
            "JOIN rooms r ON r.id = s.room_id \n" +
            "WHERE r.cinema_id = :cinemaId",
    ...
    @Query(value = "SELECT COALESCE(SUM(i.total_amount), 0) FROM invoices i \n" +
            "JOIN bookings b ON b.id::TEXT = i.booking_id \n" +
            "JOIN screenings s ON s.id = b.screening_id \n" +
            "JOIN rooms r ON r.id = s.room_id \n" +
            "WHERE r.cinema_id = :cinemaId AND i.status = :status",
