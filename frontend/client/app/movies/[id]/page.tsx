    ...
    const fetchScreenings = async () => {
      ...
      try {
        ...
        if (mapped.length > 0) {
          ...
          const availableDates = mapped.map((st) => st.date).filter(Boolean).sort();
          
          // Set to today if available, otherwise first available date
          const defaultDate = availableDates.includes(today) ? today : availableDates[0];
          
          setSelectedDate((prev) => prev || defaultDate);
          setSelectedCinema((prev) => prev || "all"); // "all" = All Cinemas
        }
      } catch (error) {
