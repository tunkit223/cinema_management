...
function checkOrphanSeatInRow(
  ...
  for (const currentIndex of selectedIndices) {
    ...
    if (getStatus(statusMap, currentIndex - 1) === 0 && getStatus(statusMap, currentIndex - 2) === 2) {
      return {
        isValid: false,
        message: `Cannot leave a single empty seat between two selected seats (Row ${sortedSeats[0].row})`
      }
    }
    if (getStatus(statusMap, currentIndex + 1) === 0 && getStatus(statusMap, currentIndex + 2) === 2) {
      return {
  ...
  if (checkCount >= 2) {
    return {
      isValid: false,
      message: `Seat selection creates multiple single empty seats (Row ${sortedSeats[0].row})`
    }
  }
  ...
  if (checkEmpty > 0 && (checkLeft > 0 || checkRight > 0)) {
    return {
      isValid: false,
      message: `Cannot leave a single empty seat when there are larger gaps available (Row ${sortedSeats[0].row})`
    }
  }
